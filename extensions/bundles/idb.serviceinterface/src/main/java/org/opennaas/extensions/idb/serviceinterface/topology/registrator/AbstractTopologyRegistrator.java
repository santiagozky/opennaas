...
public abstract class AbstractTopologyRegistrator {
	...
	public AbstractTopologyRegistrator() {
		...
		if (npString != null) {
			...
			for (int prefixNum = 0; prefixNum < numTNAPrefixes; prefixNum++) {
				this.myTNAPrefixes.add(Config.getString(
						this.getInterdomainPropertyFile(), "domain.TNAPrefix"
								+ prefixNum));
			}
		}
		...
		for (int peerNum = 0; peerNum < numPeers; peerNum++) {
			...
			final String domainName = Config.getString(
					this.getInterdomainPropertyFile(), peer + ".name");
			try {
				final String topologyEPR = Config.getString(
						this.getInterdomainPropertyFile(), peer
								+ ".topologyEPR");
				...
				this.peerDomainDistributors.put(domain.getDomainId(),
						this.createDistributor(domain));
			} catch (final MissingResourceException e) {
			...
			if (nlString != null) {
				...
				for (int linkNum = 0; linkNum < numLinks; linkNum++) {
					...
					ep.setEndpointId(Config.getString(
							this.getInterdomainPropertyFile(), link + ".srcEP"));
					i.setSourceEndpoint(ep);
					i.setDestinationDomain(domainName);
					i.setLinkID(Config.getString(
							this.getInterdomainPropertyFile(), link + ".name"));
	...
	public synchronized void contextInitialized() {
		...
		try {
			superDomainTopologyEPR = Config.getString(
					this.getInterdomainPropertyFile(), "parent.topologyEPR");
		...
		if (this.startup()) {
			...
			if (superDomainTopologyEPR != null) {
				...
				if (log != null) {
					log.debug("AbstractTopologyRegistrator: creating superDomainDistributor object");
				}
	...
	private Distributor createDistributor(final DomainInformationType peerDomain) {
		return new Distributor("PeerDistributor@" + peerDomain.getDomainId(),
				this.getLogger(), peerDomain.getDomainId(),
				peerDomain.getTopologyEPR(), null) {
			@Override
	...
	protected DomainInformationType getDomainInformation() {
		...
		domInfo.setDescription(Config.getString(
				this.getInterdomainPropertyFile(), "domain.description"));
		domInfo.setReservationEPR(Config.getString(
				this.getInterdomainPropertyFile(), "domain.reservationEPR"));
		try {
			domInfo.setTopologyEPR(Config.getString(
					this.getInterdomainPropertyFile(), "domain.topologyEPR"));
		} catch (final MissingResourceException e) {
			// don't care
		}
		try {
			domInfo.setNotificationEPR(Config.getString(
					this.getInterdomainPropertyFile(), "domain.notificationEPR"));
		} catch (final MissingResourceException e) {
			// don't care
		}
