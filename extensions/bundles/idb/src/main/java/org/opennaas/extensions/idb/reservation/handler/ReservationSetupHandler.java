...
public final class ReservationSetupHandler {
	...
	private ReservationSetupHandler() {
		if (Config.getString(Constants.idbProperties,
				"exclusiveCreateReservation").equals("true")) {
	...
	public CreateReservationResponseType createReservation(
		...
		for (ServiceConstraintType sct : element.getService()) {
			...
			} else if (sct.getTypeOfReservation().equals(
					ReservationType.DEFERRABLE)) {
				throw new OperationNotSupportedFaultException(
						"deferrable reservations not yet supported");
			}
		}
	...
	private CreateReservationResponseType createMalleableReservation(
		...
		for (Service service : resv.getServices().values()) {
			for (Connections conn : service.getConnections().values()) {
				if (conn.getStartpoint().getTNA().equals("128.1.0.10")
						&& conn.getEndpoints().iterator().next().getTNA()
								.equals("128.3.0.10")) {
					dummyUsing = true;
				}
			}
		...
		if (dummyUsing) {
			...
			ReservationSetupHandler.nrpsManager = AdapterManager
					.getMalleableInstance();
		} else {
		...
		if (null == nrpsReservations) {
			throw new ReservationFaultException(
					"Failure in reservation-processing! No feasible reservations found in PathFinder!");
		}
		...
		for (Entry<Domain, Reservation> entry : nrpsReservations.entrySet()) {
			for (Service s : entry.getValue().getServices().values()) {
				...
				for (Connections c : s.getConnections().values()) {
					Connections globalConn = globalService.getConnection(c
							.getConnectionId());
					globalConn.setMinBandwidth(c.getMinBandwidth());
					globalConn.setMaxBandwidth(c.getMaxBandwidth());

					NrpsConnections globalNC = globalConn
							.getNrpsConnection(entry.getKey());
					globalNC.setBandwidth(c.getNrpsConnection(entry.getKey())
							.getBandwidth());
				}
			}
		}
	...
	private Hashtable<Domain, CreateReservationResponseType> createResInNRPS(
		...
		try {
			nrpsResponses = ReservationSetupHandler.nrpsManager
					.createReservation(nrpsRequests);
		} finally {
			if (this.createLock != null) {
	...
	/**
	 * try to save the given reservation in the DB and create a
	...
	private CreateReservationResponseType saveResInDbAndCreateResponse(
			...
			Reservation res) throws EndpointNotFoundFaultException,
			DatabaseException, InvalidReservationIDFaultException {
		// save reservation data to DB and create response
		...
		for (final Domain domain : nrpsResponses.keySet()) {
			final MAPNRPSResvID map = new MAPNRPSResvID(res,
					(nrpsResponses.get(domain).getReservationID()), domain);
			map.save();

			// subscribe for notifications in according notification-WS
			NotificationHelpers.subscribeToWSN(map.getDomain().getName(),
					map.getnrpsReservationId());
		}
		// create topic for higher level IDB's if existent
		NotificationHelpers.createTopicForWSN(res.getReservationId());
		response.setReservationID(WebserviceUtils.convertReservationID(res
				.getReservationId()));
	...
	public IsAvailableResponseType isAvailable(final IsAvailableType element)
		...
		for (final ConnectionAvailabilityType ca : sh.getDetailedResult()) {
			...
			HashMap<Integer, ConnectionAvailabilityType> sMap = detailedResults
					.get(serviceID);
			if (sMap == null) {
			...
			if (ca.isSetBlockedEndpoints()) {
				for (String ep : ca.getBlockedEndpoints()) {
					Connections iConn = res.getService(serviceID)
