...
public final class ReservationOperationsHandler {
	...
	public ActivateResponseType activation(final ActivateType element)
		...
		if (resv == null) {
			throw new ReservationIDNotFoundFaultException("Reservation ID "
					+ element.getReservationID() + " not found");
		}
		// resv.loadServices();
		final Service service = resv.getService(element.getServiceID());
		if (service == null) {
			throw new InvalidServiceIDFaultException("Service ID "
					+ element.getServiceID()
					+ " not found (within reservation ID "
	...
	public GetStatusResponseType status(final GetStatusType element)
		...
		getPerformanceLogger().info(
				"duration_resvLoad " + (resvLoadTime - timeStart) + "ms");
		if (resv == null) {
		...
		final Hashtable<Domain, GetStatusType> nrpsRequests = resv
				.getGetStatusType();
		final long timePreNRPSManager = System.currentTimeMillis();
		getPerformanceLogger().info(
				"duration_createNRPSRequests "
						+ (timePreNRPSManager - resvLoadTime) + "ms");
		...
		getPerformanceLogger().info(
				"duration_NRPSController "
						+ (timePostNRPSManager - timePreNRPSManager) + "ms");
				...
				for (final ServiceStatusType serviceStatusType : entry
					...
					if (serviceStatusType.isSetConnections()) {
						for (final ConnectionStatusType connection : serviceStatusType
							...
							Service thisService = resv.getService(serviceId);
							Connections thisConnection = thisService
									.getConnection(connectionId);
							if (null == thisConnection) {
								throw new UnexpectedException(
										"returned connection ID '"
												+ connectionId
												+ "' doesn't match to current reservation");
							}
		...
		getPerformanceLogger().info(
				"duration_postNRPSController "
						+ (timeStop - timePostNRPSManager) + "ms");
