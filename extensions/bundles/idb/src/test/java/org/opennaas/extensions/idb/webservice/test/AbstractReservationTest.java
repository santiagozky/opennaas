...
public abstract class AbstractReservationTest extends AbstractTest {
	...
	public final void cleanUp() throws Exception {
		for (String reservationId : this.reservationIds) {
			...
			try {
				CancelReservationResponseType r = this.reservationClient
	...
	protected AbstractReservationTest() {
		super();
		this.duration = Config.getInt(Constants.testProperties,
				"default.duration");
		this.signalBuffer = Config.getInt(Constants.testProperties,
				"default.signalBuffer");
		this.rsvSignalBuffer = Config.getInt(Constants.testProperties,
		...
		for (final ServiceStatus servStatus : statusResponse.getServiceStatus()) {
			...
			for (final DomainStatusType domStatus : servStatus
				...
				Assert.assertTrue("Domain '" + domStatus.getDomain()
						...
						+ domStatus.getStatus(),
						(domStatus.getStatus().equals(status1) || domStatus
								.getStatus().equals(status2)));
			}
			Assert.assertTrue(
					"The service should contain at least 1 connection",
					servStatus.getConnections().size() > 0);
			Assert.assertTrue(
					"Service for reservation '" + reservationId
							+ "' should be active now, but it is: "
							+ servStatus.getStatus(),
					(servStatus.getStatus().equals(status1) || servStatus
							.getStatus().equals(status2)));
			for (final ConnectionStatusType conStatus : servStatus
				...
				Assert.assertTrue("Connections for reservation '"
						...
						+ conStatus.getStatus(),
						(conStatus.getStatus().equals(status1) || conStatus
								.getStatus().equals(status2)));
			}
		}
	...
	protected CreateReservationResponseType createFixedTestReservation(
		 ...
		 * .getInt(Constants.testProperties, "default.bandwidth").intValue(),
		 * Config.getInt( Constants.testProperties, "default.delay").intValue(),
		 * duration);
		 */
		CreateReservationType request = new CreateReservationType();
		request.getService().add(
				createFixedTestService(src, dst, startTime, duration));
	...
	protected CreateReservationResponseType createMalleableTestReservation(
		...
		CreateReservationType request = new CreateReservationType();
		request.getService().add(
				createMalleableTestService(src, dst, startTime, deadline,
						dataAmount));
		return this.reservationClient.createReservation(request);
	}
}
