...
public abstract class AbstractTopologyTest extends AbstractTest {
	private static final String topologyEpr = Config.getString(
	...
	protected final EditDomainResponseType editDomain(
		...
		Assert.assertTrue("EditDomain request was not successfull",
				response.isSuccess());
		return response;
	}
	...
	protected final GetDomainsResponseType getDomains() throws Exception {
		...
		Assert.assertTrue("GetDomains result-check",
				response.getDomains().get(0).getDomainId() != "");
		return response;
	}
