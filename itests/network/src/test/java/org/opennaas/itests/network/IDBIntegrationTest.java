...
/**
 * @author Jordi
 * modified by Santiago
 */
@RunWith(JUnit4TestRunner.class)
public class IDBIntegrationTest {
	...
	private static final String CAPABILIY_TYPE = "idb";
	private static final String CAPABILIY_VERSION = null;
	private static final String RESOURCE_TYPE = "IDB";
	private static final String RESOURCE_INFO_NAME = "IDB Test";
	...
	@Inject
	@Filter("(osgi.blueprint.container.symbolicname=org.opennaas.extensions.idb)")
	...
	public static Option[] configuration() {
		return options(opennaasDistributionConfiguration(),
				includeFeatures("opennaas-idb", "opennaas-ws"),
				includeTestHelper(), noConsole(), keepRuntimeFolder());
	}
	...
	public void executeQueueTest() throws ResourceException, ProtocolException {
		startResource();
		QueueCapability queueCapability = (QueueCapability) networkResource
				.getCapability(getInformation(CAPABILIY_TYPE));
	...
	private void startResource() throws ResourceException, ProtocolException {
		...
		CapabilityDescriptor queueDescriptor = ResourceHelper
				.newCapabilityDescriptor(ACTION_NAME, CAPABILIY_VERSION,
						CAPABILIY_TYPE, CAPABILITY_URI);
		lCapabilityDescriptors.add(queueDescriptor);

		// Create resource
		ResourceDescriptor resourceDescriptor = ResourceHelper
				.newResourceDescriptor(lCapabilityDescriptors, RESOURCE_TYPE,
						RESOURCE_URI, RESOURCE_INFO_NAME);
		networkResource = resourceManager.createResource(resourceDescriptor);

		// If not exists the protocol session manager, it's created and add the
		// session context
	 ...
	 * @throws ProtocolException
	 */
	private IProtocolSessionManager addSessionContext(String resourceId)
			throws ProtocolException {
	...
	private Information getInformation(String type) {
